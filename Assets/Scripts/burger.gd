...
const DEATH_DURATION = 0.5               # Time before removing dead burger
...
var is_dying := false                    # Whether burger is in death state
...
func _physics_process(delta: float) -> void:
	...
	if is_dying:
		death_timer -= delta
		if death_timer <= 0.0:
			queue_free()                 # Remove burger after death delay
...
func die() -> void:
	is_dying = true                                  # Mark burger as dying
	...
	collision_layer = 1 << 4                         # Move to physics layer 5 (bit index 4)
